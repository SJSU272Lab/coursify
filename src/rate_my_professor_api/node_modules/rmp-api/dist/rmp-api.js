   ...
   */
    var rmp = function(options) {
        ...
        priv.selectors = {
            ...
            timestamp: "td.rating > div.date",
            forCredit: "td.class > span.credit > span",
            attendance: "td.class > span.attendance > span",
            textBookUsed: "td.class > span.textbook-used > span",
            wouldTakeAgain: "td.class > span.would-take-again > span",
            gradeReceived: "td.class > span.grade > span",
            rating: "td.rating > div.rating-block > div.breakdown > div:nth-child(1) > div > span.score",
            difficulty: "td.rating > div.rating-block > div.breakdown > div:nth-child(2) > div > span.score",
            department: "#mainContent > div.right-panel > div.top-info-block > div.result-info > div.result-title"
        };
        ...
        priv.scrape = function(url, callback) {
            ...
            var scrape = function(html, callback) {
                ...
                var difficulty = []
                var data = []
                var firstname = $(priv.selectors.fname, page).text().trim()
                var lastname = $(priv.selectors.lname, page).text().trim()
                var university = $(priv.selectors.university, page).text().trim()
                var department = $(priv.selectors.department, page).text().trim()
                var re = /Professor in the (.*) department[^]*/;

                department = department.replace(re, "$1");
                // Scrape all tags
                ...
                $(priv.selectors.timestamp, page).each(function(indx, elem) {
                    timestamp.push($(elem).text().trim());
                });
                $(priv.selectors.forCredit, page).each(function(indx, elem) {
                    forCredit.push($(elem).text().trim());
                });
                $(priv.selectors.attendance, page).each(function(indx, elem) {
                    attendance.push($(elem).text().trim());
                });
                $(priv.selectors.textBookUsed, page).each(function(indx, elem) {
                    textBookUsed.push($(elem).text().trim());
                });
                $(priv.selectors.wouldTakeAgain, page).each(function(indx, elem) {
                    wouldTakeAgain.push($(elem).text().trim());
                });
                $(priv.selectors.gradeReceived, page).each(function(indx, elem) {
                    gradeReceived.push($(elem).text().trim());
                });
                $(priv.selectors.rating, page).each(function(indx, elem) {
                    rating.push($(elem).text().trim());
                });
                $(priv.selectors.difficulty, page).each(function(indx, elem) {
                    difficulty.push($(elem).text().trim());
                });
                for(var i=0; i<courses.length; i++){
                    var groupData = {};
                    groupData["Name"] = firstname + " " + lastname
                    groupData["University"] = university
                    groupData["department"] = department
                    groupData["Courses"] = courses[i];
                    groupData["TextBookUsed"] = textBookUsed[i];
                    groupData["WouldTakeAgain"] = wouldTakeAgain[i];
                    groupData["GradeRecieved"] = gradeReceived[i];
                    groupData["Message"] = comments[i];
                    groupData["timestamp"] = timestamp[i];
                    groupData["rating"] = rating[i];
                    groupData["difficulty"] = difficulty[i];
                    data.push(groupData);
                }
